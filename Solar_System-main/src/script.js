...
document.getElementById('exoplanetSearchForm').addEventListener('submit', function(e) {
    e.preventDefault();
    const input = document.getElementById('exoplanetSearchInput').value.trim().toLowerCase();
    if (!input) return;
    // Search exoplanets by name (partial match allowed)
    const found = exoplanetObjects.find(exo => exo.name.toLowerCase().includes(input));
    if (!found) {
        alert("Exoplanet not found! Please check spelling.");
        return;
    }
    // Simulate clicking that exoplanet and zooming in
    selectedPlanet = { ...found, isExoplanet: true };
    closeExoplanetInfo(); // close any open exoplanet info panel
    settings.accelerationOrbit = 0; // stop orbit movement for zoom in

    const planetPosition = new THREE.Vector3();
    found.planet.getWorldPosition(planetPosition);
    controls.target.copy(planetPosition);
    camera.lookAt(planetPosition);

    offset = 20; // camera zoom distance offset
...
function onMouseMove(event) {
    // Ignore mouse moves over UI so default behavior (like text cursor) isn't blocked
    if (
      event.target && (
        event.target.closest && (
          event.target.closest('.navbar') ||
          event.target.closest('#gui-container') ||
          event.target.closest('.planet-info') ||
          event.target.closest('.exoplanet-info')
        )
      )
    ) {
      return;
...
function onDocumentMouseDown(event) {
  // Allow UI elements (navbar, inputs, buttons, GUI, info panels) to handle clicks normally
  if (
    event.target && (
